...
class AlgorithmForm(FlaskForm):
    ...
    classes = StringField('Nombre de la columna que contiene las clases: ', validators=[
...
class ModelForm(FlaskForm):
    ...
    model = FileField('Seleccione el archivo a cargar: ', validators=[FileRequired(
        message="Archivo requerido"), FileAllowed(["sav"], "solamente se permiten archivos entregados por la aplicación")])


class TrainForm(FlaskForm):
    choicer = RadioField(
        '¿Que operación desea realizar?', validators=[DataRequired(message="Valor requerido")], choices=[('load', 'Cargar modelo previamente entrenado'), ('train', 'Entrenar modelo con dataset')])
